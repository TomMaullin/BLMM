...
def pSFS2D(XtX, XtY, ZtX, ZtY, ZtZ, XtZ, YtZ, YtY, YtX, nlevels, nparams, tol, n, init_paramVector):
  ...
  while np.abs(llhprev-llhcurr)>tol:
    ...
    for k in np.arange(len(nparams)):
      ...
      update = lam*forceSym2D(np.linalg.inv(get_covdldDk1Dk22D(k, k, nlevels, nparams, ZtZ, DinvIplusZtZD, invDupMatdict,vec=True))) @ mat2vec2D(get_dldDk2D(k, nlevels, nparams, ZtZ, Zte, sigma2, DinvIplusZtZD,vec=True))
