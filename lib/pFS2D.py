...
def pFS2D(XtX, XtY, ZtX, ZtY, ZtZ, XtZ, YtZ, YtY, YtX, nlevels, nparams, tol, n, init_paramVector):
  ...
  while np.abs(llhprev-llhcurr)>tol:
    ...
    for k in np.arange(len(nparams)):
      ...
      FisherInfoMat[p, FishIndsDk[k]:FishIndsDk[k+1]] = get_covdldDkdsigma22D(k, sigma2, nlevels, nparams, ZtZ, DinvIplusZtZD, invDupMatdict, vec=True).reshape(FishIndsDk[k+1]-FishIndsDk[k])
    ...
    for k1 in np.arange(len(nparams)):
      ...
      for k2 in np.arange(k1+1):
        ...
        FisherInfoMat[np.ix_(IndsDk1, IndsDk2)] = get_covdldDk1Dk22D(k1, k2, nlevels, nparams, ZtZ, DinvIplusZtZD, invDupMatdict,vec=True)
